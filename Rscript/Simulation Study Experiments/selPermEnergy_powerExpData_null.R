...
label_parm = 1; trueDistr = if_else(label_parm==1,"same","different")
######===========================================================-
...
switch(scenario,
       {
         ...
         if(label_parm==1){
           ...
           shiftPer = seq(1,1,length.out = 4)[shiftParm]
...

       {
         ...
         if(label_parm==1){
           ...
           cc = seq(0,0,length.out = 4)
           dat = simFromExpData.covarianceShift.null(raMatrix = dat[,-1],n1 = g1,n2 = g2,maxCov =cc[shiftParm])
         }
...

       {
         ...
         dat = data.frame(Status = labels,dat);
         fname = "WGS_meanShift";
         path_ = "/nas/longleaf/home/andrew84/selEnergyPerm2/Results/powerExpData/s3/";
         procData = processCompData(dat,minPrevalence = sparsePercent);
         dat = procData$processedData;
         impFact = procData$impFactor;
         y = dat[,-1];
